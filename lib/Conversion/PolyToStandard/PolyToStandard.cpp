...
class PolyToStandardTypeConverter : public TypeConverter {
    public:
        PolyToStandardTypeConverter(MLIRContext *ctx) {
            ...
            addConversion(
                [ctx](PolynomialTypeType type) -> Type {
...
struct ConvertMul : public OpConversionPattern<MulOp> {
    ...
    LogicalResult matchAndRewrite(MulOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
        ...
        auto polymulResult = b.create<arith::ConstantOp>(polymulTensorType, DenseElementsAttr::get(polymulTensorType, 0));
        ...
        auto outerLoop = b.create<scf::ForOp>(
            ...
            [&](OpBuilder &builder, Location loc, Value p0Index, ValueRange loopState){
                ImplicitLocOpBuilder b(op.getLoc(), builder);
                auto innerLoop = b.create<scf::ForOp>(
                    lowerBound, numTermsOp, step, loopState,
                    [&](OpBuilder &builder, Location loc, Value p1Index, ValueRange loopState) {
                        ImplicitLocOpBuilder b(op.getLoc(), builder);
                        ...
                        auto result = b.create<arith::AddIOp>(mulOp,
                            b.create<tensor::ExtractOp>(accumTensor, destIndex.getResult())
                        );
                        auto stored = b.create<tensor::InsertOp>(result, accumTensor, destIndex.getResult());
...
struct ConvertEval : public OpConversionPattern<EvalOp> {
    ...
    ConvertEval(mlir::MLIRContext *context) : OpConversionPattern<EvalOp>(context) {}

    using OpConversionPattern::OpConversionPattern;

    LogicalResult matchAndRewrite(EvalOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
        auto polyTensorType = cast<RankedTensorType>(adaptor.getPolynomial().getType());
        ...
        auto loop  = b.create<scf::ForOp>(
            ...
            [&](OpBuilder &builder, Location loc, Value loopIndex, ValueRange loopState) {
                ...
                auto result = b.create<arith::AddIOp>(mulOp, b.create<tensor::ExtractOp>(poly, coeffIndex.getResult()));
...
struct ConvertFromTensor : public OpConversionPattern<FromTensorOp> {
    ...
    ConvertFromTensor(mlir::MLIRContext *context) : OpConversionPattern<FromTensorOp>(context) {}

    using OpConversionPattern::OpConversionPattern;

    LogicalResult matchAndRewrite(FromTensorOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
        auto resultTensorTy = cast<RankedTensorType>(typeConverter->convertType(op->getResultTypes()[0]));
        auto resultShape = resultTensorTy.getShape()[0];
        auto resultElementTy = resultTensorTy.getElementType();
        ...
        if (inputShape < resultShape) {
            ...
            coeffValue = b.create<tensor::PadOp>(
                ...
                b.create<arith::ConstantOp>(rewriter.getIntegerAttr(resultElementTy, 0)),
                /*nofold=*/ false
            );
        }
...
struct ConvertToTensor : public OpConversionPattern<ToTensorOp> {
    ConvertToTensor(mlir::MLIRContext *context) : OpConversionPattern<ToTensorOp>(context) {}
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(ToTensorOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
...
struct ConvertConstant : public OpConversionPattern<ConstantOp> {
    ConvertConstant(mlir::MLIRContext *context) : OpConversionPattern<ConstantOp>(context) {}
...
struct PolyToStandard : impl::PolyToStandardBase<PolyToStandard> {
    ...
    void runOnOperation() override {
        ...
        patterns.add<ConvertAdd, ConvertSub, ConvertMul, ConvertEval, ConvertFromTensor, ConvertToTensor, ConvertConstant>(typeConverter, context);
