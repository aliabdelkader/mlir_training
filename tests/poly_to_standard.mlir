// RUN: mlir-training-opt --poly-to-standard %s | FileCheck %s
...
// CHECK-LABEL: test_lower_tensor_extend(
// CHECK-SAME:  %[[V0:.*]]: [[T:tensor<10xi32>]]) -> [[T2:tensor<20xi32>]] {
// CHECK:       %[[V1:.*]] = tensor.pad %[[V0]] low[0] high[10]
// CHECK:       return %[[V1]] : [[T2]]
func.func @test_lower_tensor_extend(%0 : tensor<10xi32>) -> !poly.poly<20> {
